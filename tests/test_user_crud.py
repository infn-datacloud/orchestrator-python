...
def test_get_user_found(session, user_id):
    ...
    with mock.patch(
        "orchestrator.v1.users.crud.get_item", return_value=fake_user
    ) as mock_get_item:
...
def test_get_user_not_found(session, user_id):
    ...
    """
    with mock.patch(
        "orchestrator.v1.users.crud.get_item", return_value=None
    ) as mock_get_item:
        result = get_user(user_id, session)
        mock_get_item.assert_called_once_with(
            session=session, entity=User, item_id=user_id
...
def test_get_users_returns_users_and_count(session):
    ...
    fake_count = 2
    with mock.patch(
        "orchestrator.v1.users.crud.get_items", return_value=(fake_users, fake_count)
    ) as mock_get_items:
...
def test_add_user_success(session):
    ...
    with mock.patch(
        "orchestrator.v1.users.crud.add_item", return_value=fake_item_id
    ) as mock_add_item:
...
def test_delete_user_calls_delete_item(session, user_id):
    ...
    """
    with mock.patch("orchestrator.v1.users.crud.delete_item") as mock_delete_item:
...
def test_get_current_user_found(session):
    ...
    fake_user = mock.Mock(spec=User)
    with mock.patch(
        "orchestrator.v1.users.crud.get_users", return_value=([fake_user], 1)
    ) as mock_get_users:
...
def test_get_current_user_not_found(session):
    ...
    user_infos.user_info = {"sub": "sub-123", "iss": "issuer-abc"}
    with mock.patch(
        "orchestrator.v1.users.crud.get_users", return_value=([], 0)
    ) as mock_get_users:
        result = get_current_user(user_infos, session)
        mock_get_users.assert_called_once_with(
            session=session,
...
def test_update_user_success(session, user_id):
    ...
    fake_user_create = mock.Mock(spec=UserCreate)
    with mock.patch("orchestrator.v1.users.crud.update_item") as mock_update_item:
...
def test_update_user_raises_no_item_to_update(session, user_id):
    ...
    fake_user_create = mock.Mock(spec=UserCreate)
    with mock.patch(
        "orchestrator.v1.users.crud.update_item",
