"""Integration tests for orchestrator.v1.users.endpoints using FastAPI TestClient."""
...
from orchestrator.exceptions import ConflictError, NoItemToUpdateError, NotNullError
...
def patch_logger(monkeypatch, mock_logger):
    ...
    monkeypatch.setattr("orchestrator.logger.get_logger", lambda *a, **kw: mock_logger)
...
def test_get_users_success(client, monkeypatch):
    ...
    monkeypatch.setattr("orchestrator.v1.users.endpoints.get_users", fake_get_users)
...
def test_delete_user_success(client, monkeypatch):
    ...
    fake_id = str(uuid.uuid4())
    monkeypatch.setattr(
        "orchestrator.v1.users.endpoints.delete_user", lambda session, user_id: None
    )
...
def test_edit_user_success(client, monkeypatch):
    ...
    monkeypatch.setattr("orchestrator.v1.users.endpoints.update_user", fake_update_user)
